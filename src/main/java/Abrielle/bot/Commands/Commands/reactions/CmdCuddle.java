...
public record CmdCuddle(Abrielle bot) implements Command {
    ...
    public void runSlash(Guild guild, TextChannel tc, Member member, SlashCommandInteractionEvent event, InteractionHook hook) throws Exception {
        ...
        hook.sendMessage(cuddle(members, roles, member)).queue();
    }
    ...
    public void runCommand(Message msg, Guild guild, TextChannel tc, Member member) throws Exception {
        ...
        tc.sendMessage(cuddle(members, roles, member)).queue();
    }

    private Message cuddle(ArrayList<Member> members, ArrayList<Role> roles, Member executor) {
        MessageBuilder message = new MessageBuilder();
        String content;

        if (roles.isEmpty() && members.isEmpty())
            content = "*Cuddles* " + executor.getAsMention();
        else {
            ...
            content = mentions + " you have been cuddled by **" + exec + "**!";
        }
